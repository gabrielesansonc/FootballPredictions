...
matches_old = pd.read_csv('MergedLeaguesMatchStats.csv')
...
# Replace Villareal for Villarreal, because there is an error in the original data
matches = matches.replace({'Villareal': 'Villarreal'}) ####### Maybe delete??
...
def get_la_liga_odds(api_key):
    ...
    next_matches_data = []
    season = '2023-24'  # Fixed season for all matches
...
next_matches_data = get_la_liga_odds('331aeb91b430fbd33e1712d535307961')
...
NN_model_categorical = Sequential([
...
NN_model_categorical.compile(optimizer=Adam(learning_rate=0.001), loss='sparse_categorical_crossentropy', metrics=['accuracy'])

# Early stopping
early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)

# Fit the model
history = NN_model_categorical.fit(
...
y_train_predNN_categorical = NN_model_categorical.predict(X_train_scaled)
y_train_predNN_categorical_rounded = np.argmax(y_train_predNN_categorical, axis=1)

# Evaluate on training data
train_accuracy_NN_categorical = accuracy_score(y_train, y_train_predNN_categorical_rounded)

# Calculate MAE for training data
train_mae_NN_categorical = mean_absolute_error(y_train, y_train_predNN_categorical_rounded)
print("Training Mean Absolute Error: ", train_mae_NN_categorical)
print("Training Custom Accuracy (0 for incorrect, 0.5 for partially correct, and 1 for correct): ", 1 - train_mae_NN_categorical / 2)
print("Training Accuracy: ", train_accuracy_NN_categorical)
print("Training Confusion Matrix: \n", confusion_matrix(y_train, y_train_predNN_categorical_rounded))
print("Training Classification Report: \n", classification_report(y_train, y_train_predNN_categorical_rounded, zero_division=1), '\n\n')

# Predict on validation data
y_val_predNN_categorical = NN_model_categorical.predict(X_val_scaled)
y_val_predNN_categorical_rounded = np.argmax(y_val_predNN_categorical, axis=1)

# Evaluate on validation data
val_accuracy_NN_categorical = accuracy_score(y_val, y_val_predNN_categorical_rounded)

# Calculate MAE for validation data
val_mae_NN_categorical = mean_absolute_error(y_val, y_val_predNN_categorical_rounded)
print("Validation Mean Absolute Error: ", val_mae_NN_categorical)
print("Validation Custom Accuracy (0 for incorrect, 0.5 for partially correct, and 1 for correct): ", 1 - val_mae_NN_categorical / 2)
print("Validation Accuracy: ", val_accuracy_NN_categorical)
print("Validation Confusion Matrix: \n", confusion_matrix(y_val, y_val_predNN_categorical_rounded))
print("Validation Classification Report: \n", classification_report(y_val, y_val_predNN_categorical_rounded, zero_division=1))
...
def plot_donut_chart(home_team, away_team, home_percentage, draw_percentage, away_percentage):
    ...
    fig.update_traces(hoverinfo='label+percent', textfont_size=14, marker=dict(line=dict(color='#000000', width=0.4)))
    ...
    for ann in fig['layout']['annotations']:
        ann['y'] = ann['y'] - 0.1  # Decrease this value to move the annotation down
...
# Iterate through each row in the DataFrame
for index, row in df_nn_predictions.iterrows():
